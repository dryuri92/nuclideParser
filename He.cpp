...
std::vector<double> transition(const std::vector<double>& xval,
 ...
 for (int i=0; i<xval.size()-1; i++) {
      ...
      if (xhalfv[i] >= xtarget[xtarget.size()-1]){
         result[xtarget.size()-1]=result[xtarget.size()-1] + yval[i];}}                            
      } else {
         while ((icurr < xtarget.size() - 1) && (xval[i] > xtarget[icurr+1])) {
             icurr++;}
         left = (xval[i] - xtarget[icurr])/(xtarget[icurr+1]-xtarget[icurr]);
         right = (-xval[i] + xtarget[icurr+1])/(xtarget[icurr+1]-xtarget[icurr]);
